...
Conventions and how-tos: root `AGENTS.md` → Frontend, plus the `alpina-frontend` skill.
The block below is a **static snapshot** generated by `bunx @tanstack/intent@latest install --map`
(run here); regenerate it after any dependency bump. Before trusting a loaded skill, compare
its `library_version` with the installed package (`bunx @tanstack/intent@latest list`).
