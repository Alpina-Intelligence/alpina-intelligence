...
import { getTaskConfig, type TaskConfig } from './config'
...
export interface SyncContext {
  ...
  /** DB config for the currently running task (set by scheduler) */
...
export const intervals = {
  ...
  schedule: (s: GameState) =>
    ({ live: HOUR, gameday: 6 * HOUR, quiet: 24 * HOUR, offseason: 24 * HOUR })[
      s
    ],
  advancedStats: (s: GameState) =>
    ({ live: HOUR, gameday: 2 * HOUR, quiet: 6 * HOUR, offseason: 24 * HOUR })[
      s
    ],
  gameLogs: (s: GameState) =>
    ({
      live: 30 * MINUTE,
      gameday: 2 * HOUR,
      quiet: 24 * HOUR,
...
export function createScheduler(
  ...
  async function taskLoop(task: SyncTask, staggerMs: number) {
    ...
    while (!signal.aborted) {
      ...
      try {
        const recordCount = await task.run({ ...ctx, taskConfig: config })
  ...
  return {
    start: async () => {
      ...
      const loops = tasks.map((task, i) => taskLoop(task, i * 2 * SECOND))
