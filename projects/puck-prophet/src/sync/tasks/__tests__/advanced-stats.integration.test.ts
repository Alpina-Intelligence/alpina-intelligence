	...
	endpoints: {
		statsReport: {
			skater: vi
				.fn()
				.mockImplementation(
					(report: string, params: { seasonId: number; gameTypeId: number; start: number; limit: number }) =>
...
vi.mock("@/lib/nhl-api/transformers", async (importOriginal) => {
	const actual = await importOriginal<typeof import("@/lib/nhl-api/transformers")>();
	return {
		...actual,
...
describe("advanced-stats task integration", () => {
	...
	it("upserts skater data to nhlSkaterAdvancedStats", async () => {
		...
		mockNhlFetch.mockImplementation(async (url) => {
			if (typeof url === "string" && url.includes("/skater/realtime") && url.includes("gt=2") && url.includes("start=0")) {
	...
	it("upserts goalie data to nhlGoalieAdvancedStats", async () => {
		const { advancedStatsTask } = await import("../advanced-stats");
		mockNhlFetch.mockImplementation(async (url) => {
			if (typeof url === "string" && url.includes("/goalie/advanced") && url.includes("gt=2") && url.includes("start=0")) {
	...
	it("paginates when response has PAGE_SIZE rows", async () => {
		...
		mockNhlFetch.mockImplementation(async (url) => {
			if (typeof url === "string" && url.includes("/skater/realtime") && url.includes("gt=2")) {
