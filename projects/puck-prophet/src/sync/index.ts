...
const ctx: SyncContext = {
	...
	setGameState(state: GameState) {
		if (state !== currentGameState) {
			console.log(
				`[sync] Game state changed: ${currentGameState} → ${state}`,
			);
...
await db
	...
	.catch((err) =>
		console.error("[sync] Failed to write engine state:", err),
	);
...
console.log(`[sync] Database: ${process.env.DATABASE_URL ? "connected" : "⚠ DATABASE_URL not set"}`);
