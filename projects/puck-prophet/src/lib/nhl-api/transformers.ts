...
export function transformStanding(
	...
	return {
		id: `${s.seasonId}-${s.teamAbbrev.default}`,
		seasonId: s.seasonId,
...
export function transformSkaterSeasonStats(
	...
	return {
		id: `${playerId}-${st.season}-${st.gameTypeId}`,
		playerId,
		season: st.season,
...
export function nhlSeasonTotals(totals: NhlSeasonTotal[]): NhlSeasonTotal[] {
	return totals.filter(
		(st) =>
			st.leagueAbbrev === "NHL" &&
			(st.gameTypeId === 2 || st.gameTypeId === 3),
	);
}
...
function advancedId(playerId: number, seasonId: number, gameTypeId: number) {
	return `${playerId}-${seasonId}-${gameTypeId}`;
}
...
export function transformSkaterRealtime(
	...
	return {
		id: advancedId(row.playerId, row.seasonId, gameTypeId),
